#!/usr/bin/env bash
# Put this machine back on an earlier version, then re-apply it.
#   sh/rollback.sh <commit-or-tag>     (see sh/history.sh)
# Only this machine changes. `make update` returns it to the latest.
...
warn "This machine is pinned to $ref. Run 'make update' to return to the latest."
